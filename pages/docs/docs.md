...
`stepWindow(int events)` limits the other half, how many of the current step's own events are kept, and it is applied on every delivery. Only an event of a declared type counts, meaning a type named by one of the flow's own `on(...)` branches or by an `event(...)` check inside a window condition.
