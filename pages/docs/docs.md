...
Where the converter or `correlateAll` threw instead, the runner never lets the subscription past the event, whatever the budget, because an event it cannot route may still belong to an instance and acknowledging it would lose it. Every instance of the saga waits behind it, the refusal is logged at `WARN` and then at `ERROR` once per interval for as long as the event keeps being offered, naming the event, the configured budget when there is one and a fixed five minutes when there is not, and once you repair the converter or `correlateAll` the event is applied in the order it was written. A broker bridge that parks the delivery instead of redelivering it gets only the first `WARN`.
