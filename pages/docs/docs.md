...
Pass your own `DataFieldReader` instead of `DataFieldReader.refusing()` to get both a payload filter and an observer. The observer runs once per event, before delivery is attempted, whether or not a handler ends up running, and independent of whether that handler goes on to succeed or throw. `outcome` is one of three values rather than a boolean, because "the filter declined this event" and "there was nothing here able to receive it" are different problems that call for opposite acknowledgement decisions. `RoutingOutcome.DELIVERED` means a running, unpaused subscription's filter accepted the event. `RoutingOutcome.FILTERED` means that same subscription evaluated the event and declined it, so redelivering it would loop forever against the same registration. `RoutingOutcome.NOT_DELIVERABLE` covers every case where no safe-to-acknowledge answer exists, whether because no running, unpaused subscription was there to reach the event at all (nothing registered, the model stopped, or the subscription paused), or because the filter itself failed before it could decide. Neither case is ever reported as `FILTERED`, since acknowledging either one risks losing an event nothing actually declined. A caller acknowledging an externally sourced event may acknowledge on `DELIVERED` once `accept(..)` has returned normally, and on `FILTERED`. It must never acknowledge on `NOT_DELIVERABLE`.
