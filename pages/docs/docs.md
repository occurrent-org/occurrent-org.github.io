...
**Implementing `CheckpointStorage` yourself is a real break, on both the blocking interface and its reactor twin.** If you implement in Java, run the `org.occurrent.UpgradeToOccurrent_0_33` OpenRewrite recipe first. It adds the three-argument `save` and `writeVersion` to every Java implementation it finds missing them, each marked with a `TODO [Occurrent 0.33 upgrade]` comment, so the module compiles again without a manual pass. The recipe is Java only, so a Kotlin implementer adds the same two members by hand, against the interface's own signatures. What it generates for a Java class, and what a Kotlin implementer writes by hand, is exactly the snippet below. `save` delegates `any()` to the existing two-argument write and refuses anything stronger, and `writeVersion` answers empty. A store that only ever wrote unconditionally can leave that exactly as generated:
...
One subscription id shape is still refused outright, an `IllegalArgumentException` from `save` for `notOlderThan` or `ifAbsent`. It is the shape Cluster itself falls back to hashing the whole id for (no brace pair, an unmatched brace, or an empty pair like `{}`), where that whole id is also either empty or contains a closing brace somewhere in it, for example `""`, `"{}orders"` or `"a}b{c"`. `any()` and `delete` never refuse an id like this, since `any()` writes only the checkpoint key. In Cluster-safe mode a conditional write refuses an id of this shape before touching Redis, so this mode itself never leaves a version key behind for one. One can still be there from `forStandalone(RedisOperations)`, which carries no such refusal and does write a version key for this shape, the case of a standalone deployment later migrated into Cluster with the same subscription ids carried over. `delete`'s fallback removes that stray version key when one is there and is a no-op when there isn't. A Redis Cluster deployment with no `CompetingConsumerStrategy` wired in is unaffected either way.

**Some storages answer per subscription id rather than once for the whole store.** `evaluatesWriteConditionsFor(subscriptionId)` is a per-id refinement of `evaluatesWriteConditions()`, defaulting to whatever that answers, for a storage whose answer depends on the id rather than being the same for every one. `SpringRedisCheckpointStorage`'s Cluster-safe constructors above are exactly this case, answering `true` overall while refusing the one subscription id shape just described. `SpringRedisCheckpointStorage.forStandalone(RedisOperations)`, for a deployment that's standalone or replicated rather than Cluster, accepts that shape too, and every other id outside the version key's own reserved namespace, since slot alignment isn't a concept a server has there; don't build a Cluster deployment's storage with it, since a conditional write for an id it accepts but Cluster can't align a slot for then fails with Redis's own `CROSSSLOT` error instead. Once every singleton exists, the Spring Boot starter's fencing check also asks `evaluatesWriteConditionsFor` for the subscription ids you've registered, and throws `CheckpointStorageCannotFenceSubscriptionException`, naming the storage and every refused id, when the answer is `false` for at least one of them.
...
A dynamic start position is read for, since it may answer the model default when the subscription starts. Only that answer refuses it, so its registration handle keeps waiting and the refusal comes out when the subscription starts.
