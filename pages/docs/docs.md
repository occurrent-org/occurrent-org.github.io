...
A live event and a firing timer do not fail the same way when a `SagaConcurrencyException` exhausts its compare-and-set retries. On the event path the exception propagates to the subscription model, and the whole step is retried wherever that model offers the event again. Whether it does is the model's own business, your listener's call on a push feed and a `DeliveryFailurePolicy` setting on a broker bridge. The subscription is one ordered channel shared by every instance the saga handles, so wherever the model does offer the event again an instance that keeps failing blocks the events queued behind it.
...
Where the instance is not quarantined, the block is the one every version up to 0.33.0 had, for as long as the model keeps offering the event. It ends when the retry succeeds, when you abandon the instance with `SagaStateStore.delete(sagaId)`, or when you stop the subscription, which stops the saga rather than only the block. [ADR 128](https://github.com/johanhaleby/occurrent/blob/main/doc/architecture/decisions/0128-a-renamed-or-removed-step-refuses-its-parked-instances.md) has the full set of remedies for an instance parked this way.
...
Only an event that reached an instance can end the block this way. Where the saga routed the event to an instance, a quarantine records whichever event the instance stopped on.

Where the converter or `correlateAll` threw instead, the runner never lets the subscription past the event, whatever the budget, because an event it cannot route may still belong to an instance and acknowledging it would lose it. For as long as the model offers the event again every instance of the saga waits behind it, the refusal is logged at `WARN` and then at `ERROR` once per interval, naming the event, the configured budget when there is one and a fixed five minutes when there is not, and once you repair the converter or `correlateAll` the event is applied in the order it was written. A model that does not offer a refused delivery again gets only the first `WARN`.
...
A saga has one subscription and every instance of that saga is fed by it, so an event that one instance cannot handle holds up every other instance behind it for as long as your subscription model offers that event again. Quarantine can put a limit on how long that lasts.

The runner times the failing rather than counting the attempts. An instance's first failure writes down when it started failing and rethrows, so a subscription model that offers the event again lets the saga try again. Where that write loses a compare-and-set that delivery records nothing, and what the next one finds is whatever the writer that won left. Once that instance has kept failing for at least `SagaRunnerConfig.quarantineAfter`, five minutes by default, it can move to `SagaStatus.QUARANTINED` on whichever event it is failing on then, and when it does the runner stops rethrowing.
...
One event that can hold the saga up stops no instance at all. The runner asks your id extractor which instance an event belongs to before anything else happens, so a converter or an id extractor that throws gives it no instance to quarantine. Such an event is never let past, whatever the budget, because it may still belong to an instance and acknowledging it would lose it. For as long as the model offers the event again every instance of the saga waits behind it, and `SagaExecution` logs a `WARN` on the first failure and an `ERROR` once per interval after that, the configured budget when there is one and a fixed five minutes when there is not, naming the event and what stopped it. A model that does not offer a refused delivery again gets only the first `WARN`.

Nothing is written for that event, so `findByStatus(QUARANTINED, ..)` does not list it. Where the event is offered again, repair the converter or the id extractor and the saga applies it in the order it was written, with nothing to feed to it again.
...
Three of the conditions need explaining before you rely on quarantine. The first is that it needs a subscription model that can be resumed at a chosen position, which means `NativeMongoSubscriptionModel` and `SpringMongoSubscriptionModel`, either of them alone or behind `DurableSubscriptionModel`, `CompetingConsumerSubscriptionModel` or `CatchupSubscriptionModel`. The wrapper alone is not enough. A model makes that promise by implementing `HistoryRetainingSubscriptions`, which is the name the startup warning gives. On any other model the runner switches quarantine off at startup and logs why, so one failing event goes back to blocking every other instance of that saga for as long as your model offers it again.
...
The third is that the model's promise is checked again for the one event in front of it. A model that implements `HistoryRetainingSubscriptions` promises to hold every event it delivers, and the runner still asks about this event before letting the subscription past, so a model that promised wrongly is caught on the event it is about to acknowledge rather than after that event is gone. Where the answer is no, that event is not quarantined, and for as long as your model offers it again the block lasts as it did before 0.34.0.
