...
One node on its own reaches the same refusal with nobody else registering, when its checkpoint storage answers from a reader that hasn't seen the write, or retries a write whose answer it never heard. Starting the node again takes whatever position storage holds by then, except for the lagging reader, which answers the same way every time and needs a reader that has seen the write instead.
