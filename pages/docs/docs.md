...
`ManualStartSubscriptionModel` in `occurrent-subscription-api-blocking` wraps any subscription model so that subscribing only registers a subscription, and nothing runs until you start it yourself. No framework is required. `ManualStartSubscriptionModel.stoppedByDefault(subscriptionModel)` withholds every subscription, then `start()` or `resumeSubscription(id)` brings one up. A registration that resolves to the subscription model's own default start position has that position fixed there, not when it's started, so events written while it waits are not skipped. A registration that names its own position instead, a replay or an explicit checkpoint, writes nothing at registration, so it keeps doing what it asked for instead of being fixed to that moment. It is also what the JUnit extension's [stopped-by-default testing](#testing-subscription-deny-by-default) is built on.
