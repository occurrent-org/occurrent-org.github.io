...
`updateEvent` keeps the values the store owns, whatever event your update function returns. Those are the event's
`position`, its DCB tags, and its `streamId` and `streamVersion`, and the event `updateEvent` returns has the same
...
The module also builds a runnable jar with the `cli` classifier, which takes the arguments
...
A later run only reports the positions it repaired itself, so keep the range from every run. A run that was killed and
resumed can report a wider range than it actually repaired, and more unrecoverable events than there really are, but
never a narrower range or fewer events.

The [repair runbook](https://github.com/johanhaleby/occurrent/blob/main/doc/runbooks/update-event-repair.md) has the
full sequence, including the queries that tell you whether you are affected, what to do with each event the tool will
not fix by itself, and how to replay or reconcile a consumer that skipped a repaired event.
...
names the event by `_id` and leaves that value alone. `UpdateEventRepairResult.unrecoverableEvents()` contains up to
1,000 findings by default (`UpdateEventRepairOptions.maxReportedUnrecoverable`), and every one is logged at `WARN`.
...
The subscription that keeps the snapshot up to date is filtered to the event types the `SnapshotView` registers handlers for. A handler on a sealed type covers every concrete type it permits, and a declared type whose concrete types cannot be found is refused at startup, which [Deriving the Event Filter](#deriving-the-event-filter) covers.
...
`accept(..)` can't refuse such an event on your behalf, the way `DomainEventFeed.accept(..)` does when nothing is registered, because it's fed from the write path as well as from a broker. [ADR 104](https://github.com/johanhaleby/occurrent/blob/main/doc/architecture/decisions/0104-an-undeliverable-push-event-is-refused-not-acknowledged.md) has the reasoning.
...
`outcome` is a `RoutingOutcome` rather than a boolean, because "the filter declined this event" and "nothing was there to receive it" are different problems, and a broker listener acknowledges the first and redelivers the second. It has six values:

| Outcome | What happened |
|:---|:---|
| `DELIVERED` | The subscription's filter accepted the event and the handler ran. Behind a `CatchupThenPushSubscriptionModel`, described further down in this section, it can also mean the event was buffered for the replay to deliver, or had already been delivered. A handler that threw still reports `DELIVERED`, and its exception propagates out of `accept(..)` once the observer has been told. |
...
| `DEFERRED` | The filter accepted the event, but the `CatchupThenPushSubscriptionModel` in front of the handler didn't deliver it, for example because its catch-up was stopped, or, through `acceptRedeliverable(..)` described below, because its replay hasn't finished. Nothing is thrown, and offering the event again later is safe. |
| `NOT_DELIVERABLE` | The filter threw instead of answering, or the `CatchupThenPushSubscriptionModel` in front of the handler refused the event for a reason that clears by itself, such as its live buffer filling up while the replay is still running. The exception propagates out of `accept(..)` either way. |
...
Offering the message again is not always enough on its own. A replay finishes by itself, but a stopped model, a paused subscription and a stopped catch-up all wait for someone to start them again, so a bridge that only redelivers keeps getting the same answer until that happens.
...
The outcome comes from the same filter check that decides where the event goes. A `stop()`, a pause or a resume that happens at the same moment therefore can't make the report disagree with what actually happened to the event.

A filter that throws a `RuntimeException` or an `AssertionError` is reported as `NOT_DELIVERABLE`, and the exception propagates out of `accept(..)` once the observer has been told. A filter can throw like that when the `DataFieldReader` you gave the model fails to read a field.

The observer isn't told at all when the filter or the handler throws a checked exception, which Kotlin code can do without declaring it, or an `Error` other than `AssertionError`. That failure propagates straight out of `accept(..)`.

Any exception your observer throws, checked ones included, is caught and logged, and so is an `AssertionError`. A broken observer therefore can't turn a delivered event into a broker redelivery, and it doesn't stop the rest of a batch from being routed. If it throws an `InterruptedException`, the calling thread is left interrupted.

An `Error` other than `AssertionError` from the observer isn't caught. When the model is already propagating a failure of its own for that event, from the filter, the handler or a catch-up refusal, your `Error` is attached to that failure with `addSuppressed(..)`, and the model's failure is what you catch. Otherwise your `Error` propagates on its own.

Without an observer, `accept(..)` skips the extra routing check the observer needs, so `PushObserver.noop()`, the default the other two constructors use, costs nothing.
...
Publication is at-least-once with no new mechanism behind it. `DurableSubscriptionModel` advances its checkpoint only after `forward`'s action returns, so a sink that throws keeps the checkpoint where it was and the event publishes again on the next run. That holds for `forward(id)` and `forward(id, filter)`. The two overloads that take a `StartAt` start where you tell them instead of resuming from the checkpoint.
...
A forwarder that already decoded a stored `CloudEvent` into a domain event, only for a sink to convert it straight back, means one decode and one re-encode per event. The result also can't preserve the id, source, subject or time the store recorded.

Forward stored events with `CloudEventForwarder` and a `CloudEventSink` instead, which converts nothing. `DomainEventForwarder` is for a `DomainEventSink<E>` you implement yourself, one that genuinely publishes domain events through its own converter.
...
At the domain level `DomainEventFeed` is the only place that decision can be made, because `accept(metadata, event)` delivers unconditionally otherwise. The domain bridge filters nothing itself. It hands every message to `acceptCloudEvent(..)`, and the feed checks each one against the filter its projection registered with, however coarse the binding is.
...
`RabbitMqCloudEventBridge` is the consuming half. It holds the `PushSubscriptionModel` itself rather than a bare `Pushable`, so it can read the model's lifecycle and acknowledge correctly.

The bridge needs the `RoutingOutcome` of each call it makes into the model, and it can't ask for it directly. A `PushObserver` is a constructor argument on `PushSubscriptionModel` with no way to attach one afterwards, while the bridge is built from a model that already exists.

`RoutingOutcomeChannel` is what connects the two. Construct one, pass it to both the model's constructor and the bridge's builder, and the bridge reads the outcome of the call it just made off that same instance.
...
RabbitMQ's own dead-lettering, where a rejected message is moved to another exchange, is not used here instead. `basicNack(requeue = false)` with no dead-letter exchange configured throws the message away outright, and even with one the broker republishes without confirms.
...
The bridge stages `record.offset() + 1` for a partition once that record resolves as `DELIVERED` or `FILTERED`, and it stages the same way for a delivery failure that resolves through a confirmed `PARK`, so all three commit that record's offset.

It commits every partition that made progress once a poll batch is walked, and never uses the no-argument `commitSync()`, which would commit the whole batch including records nothing processed yet.

On `NOT_DELIVERABLE` or a thrown exception, the same `DeliveryFailurePolicy` applies as on RabbitMQ.

`REDELIVER` seeks the consumer back to the failed record's offset and stops processing that partition's remaining records for the poll, so a later record in the same batch is never committed past one that failed, and nothing is staged for the failed one either.
...
A model that has catch-ups signals the listener twice, once when a catch-up begins, and once when it has read the history it set out to read. A catch-up that is stopped before then sends only the first. A projection that [records the appends it has applied](#projection-annotation-applied-appends) records nothing between those two signals, and records again after the second one. Some of the events written while a catch-up ran are delivered by that catch-up and never again, so they still have to be recorded. `CatchupThenPushSubscriptionModel` also calls the listener's `alreadyDeliveredByReplay(event)` when the live feed offers a copy of an event its replay already delivered, which is how an append whose only delivery was the replay's gets recorded.
...
* The observer doesn't always run on the thread that called `accept(..)`. `UNAVAILABLE`, `FILTERED` and a filter that threw are reported on the thread that subscribed to the returned `Mono`. The other outcomes are reported once your handler, or the `CatchupThenPushSubscriptionModel` in front of it, has finished with the event, on whichever thread that happened. An `InterruptedException` from the observer sets the interrupt flag again on that thread, and a pooled Reactor worker clears the flag before its next task, so the caller may never see it.
...
`subscribe(..)` derives the subscription filter from the event types you name, so a sealed type covers every concrete type it permits, and a type whose concrete types cannot be found is refused at that call. See [Deriving the Event Filter](#deriving-the-event-filter).
...
`query(GameStarted.class, GameEnded.class)` and the `Collection` overloads derive a filter from the types you list, so a sealed type covers every concrete type it permits, and a type whose concrete types cannot be found is refused on each query that names it. See [Deriving the Event Filter](#deriving-the-event-filter). A `null` or empty collection still means "match nothing" rather than "match everything".
...
The reactive MongoDB store is the exception to that. Call its `waitUntilApplied` on a thread Reactor marks non-blocking, a WebFlux handler for example, and it throws `IllegalStateException` straight away. Call it from `Schedulers.boundedElastic()` or another thread meant for blocking work.
...
Occurrent then wraps the projection's store, so the projection records the append of every event it applies. Replays are the exception, and they are covered below.
...
Without Spring Boot you do the same by hand. `AppliedAppendStore.inMemory()` gives you a store for tests or for a single-process application. It keeps the 10000 most recent appends per projection, and a wait for one it has dropped times out. `Projections.recordingAppliedAppends(view, projectionId, store)` wraps the projection, on the blocking stack and the reactor stack alike.

Nothing tells that wrapper when a catch-up begins and ends unless you arrange it. Pass it to [`listenForCatchup`](#subscription-model-capabilities) on the subscription model the projection runs on, before you subscribe.

On the blocking stack, `ReplayAwareSubscriptions.findIn(subscriptionModel)` finds that model through any models wrapping it. The reactor stack has no `findIn`. Check the model with `instanceof` instead, and ask the subscription model itself, since the subscription DSL wrappers don't pass the capability on.

If `listenForCatchup` returns `false`, you have to watch the model yourself. Call `catchupStarted` on the wrapper with a new object when `isCatchingUp(projectionId)` turns `true`, and `historyRead` with that same object when it turns `false` again. A wrapper that nobody tells records straight through a replay and never clears.

Also call the wrapper's `pollForClear()` on a schedule, on a thread meant for blocking work. It retries a clear that failed because the store was unavailable when a catch-up began. Without it, that clear is only retried when another event reaches the projection.
...
| `replay-poll.initial` / `.max` / `.multiplier` | `200ms` / `5s` / `2.0` | How often the starter retries a clear a catch-up still owes, and on a subscription model that doesn't send catch-up signals, how often it checks whether the projection is catching up. Both are described below. |
...
A push projection (`source = PUSH`) doesn't use `startAt`. Its `catchup` decides instead. The default, `FROM_EVENT_STORE`, replays history before the projection goes live, and `NONE` never replays.
...
A projection left at `StartPosition.DEFAULT` never replays on that composition, so it never clears its own records. Wipe its read model, then call `AppliedAppendStore.clear(projectionId)` yourself. A projection that is running when you do that isn't told, so if the append it recorded last is delivered again it skips recording it, and a wait for that one append times out.

The Spring Boot starter logs a `WARN` at startup naming a `recordAppliedAppends = true` projection it can tell never replays. That covers an explicit `NOW`, a `DEFAULT` on Occurrent's own MongoDB composition, a push projection with `catchup = NONE`, and on the reactor stack a composition with no catch-up layer. It can't tell for a subscription model of your own, so it stays silent there.

A projection whose start position does replay clears its recorded appends when the catch-up begins, and starts recording again only after the history has been read. A wait for an append recorded before that clear times out rather than answering `true` about a read model the rebuild has already wiped.

Those two moments aren't the same instant. Recording stops when the catch-up begins, and the old records stay readable until the clear finishes, so a wait running in between can still get `true` for an append whose read model the rebuild is discarding.
...
Occurrent's own subscription models tell the projection a catch-up has begun before it delivers anything, so no replay goes unnoticed on them. The clear itself runs against the store, and if the store is unavailable at that moment, the clear is retried on the next event the projection receives and on the `replay-poll` schedule. That poll comes with the Spring Boot starter. Without it, schedule `pollForClear()` yourself as described above.

A subscription model that doesn't send the catch-up signals puts the projection on a fallback. The starter checks `isCatchingUp` on the `replay-poll` schedule, and the projection records nothing until the catch-up is over, including the writes that arrive while it runs.

A catch-up that starts and finishes between two of those checks isn't seen at all, so the projection records the history it replayed as if it were live, and its old records survive it. The checks start at 200ms and back off to 5 seconds, so once they are that far apart, a catch-up shorter than 5 seconds can be missed.
...
Where the concrete types cannot be found, `build()` throws `IllegalArgumentException` naming the type. [Deriving the Event Filter](#deriving-the-event-filter) lists every declaration whose concrete types cannot be found, including a concrete class that is neither final nor sealed. A `sealed class` you can instantiate is no exception, because a sealed declaration says its subtypes are knowable whether or not events are stored under the root's own name.
...
`DomainEventQueries.query(Collection)` and its sibling overloads treat a `null` or empty collection as "match nothing" and return an empty result. A collection is only expanded when it has something in it, so an empty one is never turned into `Filter.all()` the way an empty `eventTypes()` is for a projection, a subscription or a snapshot, which match everything.
...
The hierarchy has to be sealed or final all the way down, and a declared type whose concrete types cannot be found is refused at startup. The message names the type and points you at `eventTypes()`. [Deriving the Event Filter](#deriving-the-event-filter) lists every declaration that cannot be found, including a concrete class that is neither final nor sealed.

`@Saga` derives its filter from the types the saga declares and refuses the same declarations when the saga is built, which [Declared Event Types](#saga-event-types) covers. `@Projection` and `@Snapshot` derive theirs from the types you register with `on(...)` and refuse them at startup, and so do the [subscription DSL](#subscription-dsl) and `DomainEventQueries` at the call that needs a filter.
