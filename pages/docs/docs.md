...
`isReplayingHistory(subscriptionId)` splits that answer in two. A catch-up reads the history that was already there and then delivers whatever was written while it was reading, and this answers `false` for the second part while `isCatchingUp` still answers `true`. Occurrent's own models answer it. Implement it on a model of your own that does the same thing, because the default answers `isCatchingUp`, and a `recordAppliedAppends` projection reading that default records nothing until the handover.
