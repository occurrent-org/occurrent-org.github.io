...
* Kotlin users now also get clearer collection-oriented names such as `executeSequence(...)` and `executeList(...)`, plus direct-import helpers like `sideEffect(...)` and `filter(...)`, and namespaced typed filters under `ExecuteFilters`.
