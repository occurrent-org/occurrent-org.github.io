...
This methods are available in the new `SubscriptionModelLifeCycle` interface and supports canceling, pausing and resuming individual subscriptions. 
...
Another big change a much improved `RetryStrategy`. You can now configure max attempts, a retry predicate, error listener as well as the backoff strategy.
