...
`DELIVERED` and `FILTERED` are the two settled answers. A running subscription either accepted the event or declined it under its filter, and either way its fate is decided. The other four, `UNAVAILABLE`, `NOT_DELIVERABLE`, `DEFERRED` and `REFUSED`, each leave it undecided, for a different reason.
