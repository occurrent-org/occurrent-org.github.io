...
| View DSL `evolve` | `View.evolve`, `evolveAll`, and `evolveFrom` dropped their `Stream` overload. They still accept a `List`, `Sequence`, `Iterable`, or varargs. | Convert a `Stream` argument to one of those. A `List`, `Sequence`, or varargs call needs no change. |
